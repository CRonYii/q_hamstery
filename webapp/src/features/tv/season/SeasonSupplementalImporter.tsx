...
import { b64DecodeUnicode, getEpNumber, isSupplementalFile } from '../../../app/utils';
...
import { seasonActions, seasonSupplementalImportSelector } from './seasonSlice';

const GlobalSeasonSupplementalImporter: React.FC = () => {
  const info = useSelector(seasonSupplementalImportSelector)
  if (!info.season)
    return <div />
  return <div>
    <ApiLoading getters={{
    ...
    }}>
      {
        ({ values }) => {
          ...
          return <SeasonSupplementalImporter season={info.season as ITvSeason} episodes={episodes} />
        }
      }
    </ApiLoading>
  </div>
...
const SeasonSupplementalImporter: React.FC<{
  season: ITvSeason,
  episodes: ITvEpisode[],
}> = ({ season, episodes }) => {
  ...
  const chooseTab = <div>
    <PathSelectorV2Modal type='path' onChange={async (opt) => {
      ...
      setFiles(data.file.filter((f) => isSupplementalFile(f.title)));
      setImports([])
    }} />
    <Table
      rowKey='title'
  ...
  const importTab = <Form
    layout='vertical'
    id="importSeasonSupplemental"
    name="importSeasonSupplemental"
    labelCol={{ span: 24 }}
    onFinish={async (data: { episodes: { episode_number: number, path: string }[], mode: string }) => {
      try {
        ...
        notification.success({ message: 'Successfully imported suuplemental files' })
      } catch {
        notification.error({ message: 'Failed to import suuplemental files' })
      }
    }}
  >
    ...
    <Button key='submit' form="importSeasonSupplemental" type="primary" loading={isLoading} htmlType="submit">Import</Button>
  </Form>

  const items = [
    {
      label: 'Choose supplemental files',
      key: 'choose',
      children: chooseTab
    },
  ]
...
export default GlobalSeasonSupplementalImporter