...
def get_episode_number_from_title(title: str, force_local=False) -> int:
    ...
    if not force_local and settings.openai_title_parser_mode == HamsterySettings.TitleParserMode.PRIMARY:
        ep = openai_manager.get_episode_number_from_title(settings.openai_title_parser_model, settings.openai_title_parser_prompt, title)
        if ep:
            return ep
    try:
        ep = int(title)
    ...
    if not match:
        if not force_local and settings.openai_title_parser_mode == HamsterySettings.TitleParserMode.STANDBY:
            return openai_manager.get_episode_number_from_title(settings.openai_title_parser_model, settings.openai_title_parser_prompt, title)
        return None
