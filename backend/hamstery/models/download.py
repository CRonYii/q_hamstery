...
class DownloadManager(models.Manager):
    ...
    def import_download(self, qbt_task):
        ...
        qbt.client.torrents_set_category(
            category=HAMSTERY_CATEGORY, torrent_hashes=[task.hash])
...
class TvDownload(PolymorphicModel):
    ...
    def map_files_to_episodes(files):
        ep_heap = []
        episodes_map = utils.get_best_episode_from_titles(
            files, lambda f: f['name'])
        for k in episodes_map:
            heapq.heappush(ep_heap, k)
        order_eps = list()
        while len(ep_heap):
            ep_n = heapq.heappop(ep_heap)
            order_eps.append(episodes_map[ep_n])
        ...
        if len(files) == 1:
            ...
            for f in ep_files:
                if f['episode'] == self.episode.episode_number:
                    target_file = f['entity']
                    break

        if target_file is None:
            return self.fail('Failed to locate a video file for episode %s' % (self.episode.episode_number))
    ...
    def parse_files(self, files):
        ...
        for f in ep_files:
            ...
            if last_ep != -1 and last_ep + 1 != f['episode']:
                # need ep_files in order because we want to check if all files's episode are continuous
                continuous = False
            last_ep = f['episode']
        ...
        if len(ep_files) > 1 and continuous:
            ...
            for f in ep_files:
                episode_number = f['episode'] - self.subscription.offset
                if episode_number == self.episode.episode_number:
                    continue
                # We need all files in this download match a corresponding episode
                episode_query = TvEpisode.objects.filter(
                    season=self.subscription.season, episode_number=episode_number)
                if not episode_query.exists():
                    auto_matching_qualified = False
                    break
                f['episode'] = episode_query.first()
                to_be_created.append([
                    episode_query.first(), f['entity']
                ])

            if auto_matching_qualified:
                for [episode, file] in to_be_created:
                    file_index = file['index']
                    filename = file['name']
                    MonitoredTvDownload.objects.create(
                        task=self.task,
                        episode=episode, file_index=file_index, filename=filename,
                        auto_matched=True,
                        subscription=self.subscription)
                    logger.info('Auto matched Episode "%s" for download "%s"' % (
                        episode, self.task.name))
